...
[`reitit-extras`](https://github.com/abogoyavlensky/reitit-extras) small helper library for convenience. It offers several options to extend the server configuration.
...
The database file will be created at the root directory of the project with the name `myproject.sqlite`.
...
First, let's add SQL queries for creating and retrieving movies. Create a new file `src/myproject/queries.clj`:
...
The single parameter of a handler is the `request` map. Each handler automatically receives a `:context` key added to the request by the [`wrap-context`](https://github.com/abogoyavlensky/reitit-extras/blob/32bde6d826603a3a0084d3266c5b936e7b802562/src/reitit_extras/core.clj#L182) middleware,
...
And last thing. To show the home page properly let's update the handler for it passing `router` and `movies` list from the database instead of static value:
...
We need to pass a couple of important parameters to our components:

1. The `router` to the page view function, allowing us to build URLs for creating movies by name
2. The `movies` list to render the table content
...
To enable secure `POST` requests, we've added a hidden input with a CSRF token `(reitit-extras/csrf-token-html)`.
...
Our form can create a movie, but what happens if we don't fill in the title field? It return an error because this field is required in the database.
...
After reseting the system in the REPL `(reset)` and refreshing the page, we can see the form validation errors when we try to create a movie without filling in the required fields:

![Create form validation errors](@/public/lite_create_errors.png)

## Delete a movie

To delete a movie let's add query, handler and route for it. The route will look like this:
...
For real-world application you might want to add more features like: user registration and authentication,
